  ...
  <div class="flex flex-col items-center justify-center min-h-[100dvh] p-4 bg-gray-50 dark:bg-black">
    <div class="w-full max-w-md space-y-8">
      <div class="text-center">
        ...
        <p class="text-gray-500 dark:text-gray-400 text-sm">{{ t('auth.welcome') }}</p>
      </div>
      ...
      <UCard :ui="{ body: 'p-6 md:p-8', rounded: 'rounded-2xl' }">
        ...
        <template #footer>
          <div class="text-center text-sm text-gray-500">
            {{ t('auth.noAccount') }}
            <NuxtLink to="/register" class="text-primary font-semibold hover:underline">{{ t('auth.register') }}</NuxtLink>
          </div>
        </template>
      </UCard>
    </div>
